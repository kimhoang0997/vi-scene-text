...
def main():
    ...
    with open(args.labels_path,'r',encoding='utf-8') as label_file:
        lines = label_file.readlines()
        data = {}
        for line in lines:
    ...
    while index < len(dataset):
        ...
        left_bottom = [0,0]
        right_bottom = [image.shape[1],0]
        right_top = [image.shape[1],image.shape[0]]
        left_top = [0,image.shape[0]]
        roi = [[left_bottom, right_bottom, right_top,left_top]]   

        label = [data[image_name]]
        index = debugger(image_name, image, roi,label, args.watch, args.rate, index)
